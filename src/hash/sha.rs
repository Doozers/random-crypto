...
impl Input {
    pub fn new(src: &str, variant: Variant) -> Self {
        Self {
    ...
    fn init(&self) -> Vec<u8> {
        ...
        let rest = len + (chunk_size - len % chunk_size);
        res.resize(ternary!(rest - len > 8, rest, rest + chunk_size), 0);
        res[len] = 1 << 7;
    ...
    fn process(&self, pre_process: Vec<u8>) -> U256 {
        let final_hash = pre_process.chunks(64).fold(
            match self.variant {
                Variant::SHA256 => H256,
                Variant::SHA224 => H224,
            },
            |v, batch| self.round(batch, v),
        );

        let digest: &[u32] = match self.variant {
            Variant::SHA256 => &final_hash,
            Variant::SHA224 => &final_hash[0..7],
        };

        digest
            .iter()
            .enumerate()
            .fold(U256::from(0u8), |acc, (i, v)| {
                acc | (U256::from(*v)
                    << ((match self.variant {
                        Variant::SHA256 => 7,
                        Variant::SHA224 => 6,
    ...
    fn round(&self, batch: &[u8], prev_h: [u32; 8]) -> [u32; 8] {
        ...
        for i in 0..64 {
            ...
            let temp1: u64 =
                (curr_h[H_ as usize] as u64 + s1 + ch + K[i] as u64 + new_batch[i] as u64)
                    % 2_u64.pow(32);
            ...
            curr_h[E_ as usize] =
                ((curr_h[D_ as usize] as u64 + temp1) as u64 % 2_u64.pow(32)) as u32;
